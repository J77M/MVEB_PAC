% creates legend with BA areas for 3D plot of MarsAtlas
...
save_path = "/home/jur0/project_iEEG/code/figs/OUT/MarsAtlasLegendBA";
...
for s=1:length(AtlasLabels)
    if strcmp(lobes{s}, 'Subcortical')
        ...
        displayNames{s} = sprintf('%s - %s', AtlasLabels{s}, T.BrodmanArea{s});
    end
...
tl = tiledlayout(5,1, 'Padding', 'none', 'TileSpacing', 'none'); 
ax1 = nexttile([4 1]);
...
leg = legend(dummy, 'Orientation', 'vertical', 'NumColumns', 3, 'Location', 'north');
...
title(tl, "Brodmann areas (BA) of MarsAtlas labels")
